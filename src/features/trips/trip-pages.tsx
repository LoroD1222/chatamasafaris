...
const amberButton =
  "rounded-[10px] bg-[#E0B880] text-[#403028] shadow-none transition hover:bg-[#C99A5D]";
        ...
        <section className={`${pageContainer} pb-[116px] pt-[64px] md:pt-[62px]`}>
          <div className="text-center">
            <p className="text-[13px] font-semibold leading-[1.6]">
              <Link href="/trips" className="text-[#403028]/75">Tanzania</Link>
              <span className="mx-3 text-[#403028]/45">&gt;</span>
              <span className="font-bold text-[#403028]">Safari</span>
            </p>
            <h1 className="mt-8 text-[34px] font-semibold leading-[1.15] tracking-[-0.01em] text-[#403028] md:text-[43px]">
          ...
          <div className="mt-[34px] text-center">
            <p className="inline-flex rounded-full border border-[#E0B880] bg-[#FFF8F0] px-5 py-2 text-[12px] font-bold leading-none text-[#E0B880]">
          ...
          {filteredCards.length > 0 ? (
            ...
            <div className="mx-auto mt-[57px] max-w-[680px] rounded-[8px] border border-[#403028]/12 bg-white p-8 text-center shadow-[0_14px_34px_rgba(64,48,40,0.08)]">
              <p className="text-[18px] font-bold leading-[1.35]">No trips match those filters.</p>
              <button type="button" onClick={resetFilters} className="mt-4 text-[14px] font-bold text-[#E0B880] underline underline-offset-4">Reset filters</button>
            </div>
          )}
          <p className="mx-auto mt-[88px] max-w-[960px] text-center text-[15px] font-semibold leading-[1.6] text-[#403028]/70">
...
export function TripDetailPage({ dictionary, trip, similarTrips = [] }: TripDetailPageProps) {
  ...
  const seasons = trip.bestTimeSeasons && trip.bestTimeSeasons.length > 0
    ? trip.bestTimeSeasons.map((s, i) => ({ title: s.period, dot: ["#FFF8F0","#FFF8F0","#E0B880"][i] || "#E0B880", description: s.highlight }))
    : [{ title: "Jul - Oct", dot: "#FFF8F0", description: "Peak migration. River crossings most frequent. Best overall wildlife." }];
      ...
      <main>
        ...
        <div className="relative">
          ...
          <section id="tour-details" className="mx-auto grid max-w-[1200px] scroll-mt-[82px] gap-8 px-6 pb-[48px] md:pb-[84px] lg:grid-cols-[minmax(0,879px)_1fr]">
            ...
            <aside className="relative z-[100] self-center rounded-[10px] border border-[rgba(224,123,57,0.25)] bg-white p-8 shadow-[0_20px_45px_rgba(64,48,40,0.12)] lg:-ml-[200px] lg:w-[calc(100%+200px)]">
              <h2 className="border-b border-[#403028]/18 pb-5 text-[24px] font-semibold leading-[1.18]">Is this tour for me?</h2>
              <div className="border-b border-[#403028]/13 py-5">
              ...
              <p className="border-b border-[#403028]/13 py-5 text-[13px] font-semibold leading-[1.65] text-[#403028]/70">
...
function TripBreadcrumb() {
  return (
    <nav className="mb-[47px] text-[13px] font-semibold leading-none text-[#403028]/55" aria-label="Breadcrumb">
      <Link href="/trips" className="transition hover:text-[#403028]">Tanzania</Link>
      <span className="mx-2 text-[#403028]/35">&gt;</span>
      <Link href="/trips" className="transition hover:text-[#403028]">Safari</Link>
      <span className="mx-2 text-[#403028]/35">&gt;</span>
      <span className="text-[#403028]/80">Great Migration</span>
    </nav>
  );
}
...
function TripFilterBar({ tripTypes, selectedTypes, minPrice, maxTripPrice, maxPrice, resultCount, onToggleType, onPriceChange, onResetFilters }: { tripTypes: string[]; selectedTypes: string[]; minPrice: number; maxTripPrice: number; maxPrice: number; resultCount: number; onToggleType: (type: string) => void; onPriceChange: (price: number) => void; onResetFilters: () => void; }) {
  return (
    <div className="mx-auto mt-[72px] grid max-w-[766px] overflow-hidden rounded-[2px] border border-[#403028]/12 bg-white md:grid-cols-[1fr_1.15fr]">
      <div className="border-b border-[#403028]/12 px-5 py-4 md:border-b-0 md:border-r">
        <div className="flex items-center justify-between gap-3">
          <p className="text-[12px] font-bold leading-[1.4]">Trip type:</p>
          {selectedTypes.length > 0 ? (
            <button type="button" onClick={onResetFilters} className="text-[11px] font-bold leading-none text-[#E0B880] underline underline-offset-2">Clear</button>
          ) : null}
        </div>
        <div className="mt-4 flex flex-wrap gap-2.5">
          {tripTypes.map((type) => {
            ...
            return (
              <button key={type} type="button" onClick={() => onToggleType(type)} className={`inline-flex h-[28px] items-center gap-2 rounded-full border px-3 text-[11px] font-bold leading-none transition ${selected ? "border-[#E0B880] bg-[#FFF8F0] text-[#403028]" : "border-[#403028]/12 bg-[#FFF8F0] text-[#403028]/62 hover:border-[#E0B880]"}`} aria-pressed={selected}>
                <span className={`grid size-[14px] place-items-center rounded-full border ${selected ? "border-[#E0B880] bg-[#E0B880] text-white" : "border-[#403028]/20"}`}>
      ...
      <div className="px-5 py-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <p className="text-[11px] font-bold leading-none text-[#403028]/58">{resultCount} trips</p>
        </div>
        <div className="mt-4">
          <div className="relative h-[14px]">
            <div className="absolute left-0 right-0 top-1/2 h-[3px] -translate-y-1/2 bg-[#E0B880]/55" />
            <div className="absolute left-0 top-1/2 size-[14px] -translate-y-1/2 rounded-full bg-[#E0B880]" />
            ...
            <div className="absolute top-1/2 size-[14px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#E0B880]" style={{ left: `${((maxPrice - minPrice) / (maxTripPrice - minPrice)) * 100}%` }} />
          </div>
          <div className="mt-2 flex justify-between text-[12px] font-semibold text-[#E0B880]">
  ...
  return (
    <div className={`mx-auto grid max-w-[1111px] gap-x-[18px] gap-y-[14px] md:grid-cols-2 lg:grid-cols-3 ${className}`}>
      {visibleCards.map((trip, index) => (
        <Link key={`${trip.slug}-${index}`} href={`/trip/${trip.slug}`} className="group flex h-[401px] flex-col overflow-hidden rounded-lg bg-astra-cocoa text-white shadow-[0_18px_55px_rgba(64,48,40,0.15)] outline-none transition focus-visible:ring-2 focus-visible:ring-astra-gold focus-visible:ring-offset-2 focus-visible:ring-offset-[#FFF8F0]">
          ...
          <div className="bg-[#403028] px-5 pb-[25px] pt-5 text-white">
            <h2 className="text-sm font-bold leading-[1.6]">{trip.title} - <span className="font-semibold">{trip.duration}</span></h2>
            <p className="mt-2 text-[13px] font-semibold leading-[1.45] text-white/68">{trip.route} - {trip.season}</p>
            <span className="mt-4 inline-flex h-[27px] items-center gap-1 rounded-full bg-astra-gold px-3 text-sm font-semibold leading-[1.6] text-astra-cocoa transition group-hover:bg-astra-gold/90">See Itinerary<ArrowRight className="size-3.5" aria-hidden="true" /></span>
          </div>
        </Link>
      ))}
    </div>
  ...
  return (
    <div className={className}>
      <div className="relative aspect-square overflow-hidden rounded-[2px] bg-[#403028] lg:aspect-auto lg:h-[574px]">
        ...
        <button type="button" onClick={showPreviousImage} className="absolute left-4 top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-full bg-white/92 text-[#403028] shadow-[0_10px_24px_rgba(0,0,0,0.2)] transition hover:bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--astra-primary-amber)]" aria-label="Show previous safari image"><ChevronLeft className="size-5" aria-hidden="true" /></button>
        <button type="button" onClick={showNextImage} className="absolute right-4 top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-full bg-white/92 text-[#403028] shadow-[0_10px_24px_rgba(0,0,0,0.2)] transition hover:bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--astra-primary-amber)]" aria-label="Show next safari image"><ChevronRight className="size-5" aria-hidden="true" /></button>
  ...
  return (
    <span className="flex items-center gap-1" aria-label="5 star rating">
      {Array.from({ length: 5 }).map((_, index) => (
        <Star key={index} className={`${className} fill-[#E0B880] text-[#E0B880]`} aria-hidden="true" />
      ))}
    </span>
  );
}
...
function TripSummaryCard({ dictionary, trip, className = "" }: { dictionary: HomeDictionary; trip: TripDetail; className?: string }) {
  return (
    <aside className={`flex rounded-[10px] border border-[#403028]/12 bg-white shadow-[0_22px_44px_rgba(64,48,40,0.08)] ${className}`}>
      <div className="flex min-h-full w-full flex-col px-6 py-6">
        <div className="grid gap-3 border-b border-[#403028]/13 py-5">
          <div className="flex items-center justify-between rounded-[8px] bg-[var(--astra-cream-panel)] px-4 py-3">
            <span className="flex items-center gap-2 text-[13px] font-bold uppercase tracking-[0.05em] text-[#403028]/62"><TrendingUp className="size-4 text-[var(--astra-primary-amber)]" aria-hidden="true" />In Popularity</span>
          ...
          <div className="flex items-center gap-3 rounded-[8px] border border-[#403028]/10 px-4 py-3 text-[16px] font-bold">
        ...
        <dl className="grid gap-4 border-b border-[#403028]/13 py-5 text-[16px] leading-[1.6]">
          <div className="grid grid-cols-[140px_1fr]">
            <dt className="font-semibold text-[#403028]/65">Physical rating</dt>
            <dd className="font-bold">{trip.physicalRating || 'Easy'}</dd>
          </div>
          <div className="grid grid-cols-[140px_1fr]">
            <dt className="font-semibold text-[#403028]/65">Tour Start</dt>
            <dd className="font-bold">{trip.season || 'All seasons'}</dd>
          </div>
          <div className="grid grid-cols-[140px_1fr]">
            <dt className="font-semibold text-[#403028]/65">Duration</dt>
            <dd className="font-bold">{trip.duration}</dd>
          </div>
        </dl>
        <p className="mt-5 text-[15px] font-semibold leading-[1.65] text-[#403028]/66">
          {trip.shortDescription || 'This morning is yours to enjoy at a relaxed pace.'}
        </p>
        <div className="mt-8 border-t border-[#403028]/13 pt-5">
...
function BuiltForContent({ trip, className = "" }: { trip: TripDetail; className?: string }) {
  ...
  return (
    <section className={className}>
      ...
      <p className="mt-[29px] max-w-[663px] text-[16px] font-semibold leading-[1.65] text-[#403028]/70">
        Private guided Tanzania safaris - planned for you, priced in USD, backed by 15 years of getting Americans to Africa.
      </p>
      <div className="mt-[25px] grid gap-6 sm:grid-cols-2">
        {items.map(({ label, Icon }) => (
          <p key={label} className="flex items-center gap-4 text-[18px] font-bold leading-[1.5]">
            <Icon className="size-9 shrink-0 text-[#E0B880]" strokeWidth={1.8} aria-hidden="true" />
            {label}
          </p>
        ))}
      </div>
...
function ReviewContactCard({ dictionary, trip, className = "" }: { dictionary: HomeDictionary; trip: TripDetail; className?: string }) {
  ...
  return (
    <aside className={`self-start rounded-[10px] border border-[#403028]/12 bg-white p-7 shadow-[0_16px_36px_rgba(64,48,40,0.06)] ${className}`}>
      <blockquote className="border-b border-[#403028]/13 pb-5">
        <RatingStars className="size-4" />
        <p className="mt-4 text-[17px] font-semibold italic leading-[1.65] text-[#403028]/75">
          &ldquo;{trip.featuredReviewQuote || "The river crossing was unlike anything I've ever seen in my life."}&rdquo;
        </p>
        <footer className="mt-4 text-[14px] font-bold leading-none text-[#403028]">
          {reviewAuthor}
        </footer>
      </blockquote>
      <div className="pt-5">
        <p className="text-[15px] font-semibold leading-[1.6]">For extra information please contact us</p>
        <p className="mt-3 flex flex-wrap gap-x-5 gap-y-3 text-[13px] font-bold leading-[1.6] text-[#403028]/65">
          <a href={`https://wa.me/${dictionary.topBar.phone.replace(/\D/g, "")}`} className="inline-flex items-center gap-2 hover:text-[#E0B880]"><Phone className="size-4 text-[#E0B880]" aria-hidden="true" />{dictionary.topBar.phone}</a>
          <a href={`mailto:${dictionary.topBar.email}`} className="inline-flex items-center gap-2 hover:text-[#E0B880]"><Mail className="size-4 text-[#E0B880]" aria-hidden="true" />{dictionary.topBar.email}</a>
        </p>
      </div>
    </aside>
  );
          ...
          <div className="grid h-full w-full grid-cols-5 items-center">
            {tripTabs.map((tab) => (
              <a key={tab.label} href={tab.href} onClick={() => setActiveTab(tab.href)} className={`relative flex h-full items-center justify-center px-1 text-center transition hover:text-[#403028] ${activeTab === tab.href ? "text-[#403028]" : ""}`}>
                {tab.label}
                {activeTab === tab.href ? <span className="absolute bottom-0 left-1/2 h-[4px] w-[min(100%,72px)] -translate-x-1/2 rounded-t-full bg-[#E0B880]" /> : null}
              </a>
            ))}
          </div>
        </nav>
...
function OverviewText({ trip }: { trip: TripDetail }) {
  return (
    <div>
      ...
      <p className="whitespace-pre-line text-[15px] font-semibold leading-[1.68] text-[#403028]/74">
    ...
    <section id="itinerary" className={`${pageContainer} scroll-mt-[82px] py-[48px] md:py-[72px]`}>
      ...
      <div className="mt-[32px] grid gap-6 md:mt-[56px]">
        {days.map((day, index) => {
          ...
          return (
            <article key={`${day.day}-${day.title}`} className="grid overflow-hidden rounded-[8px] border border-[#403028]/10 bg-white shadow-[0_18px_45px_rgba(64,48,40,0.08)] lg:grid-cols-[420px_minmax(0,1fr)]">
              <ItineraryImage images={[dayImage]} sizes="420px" />
              <div className="px-6 py-8 lg:order-2 lg:px-10 lg:py-9">
                <p className="text-[13px] font-bold uppercase leading-none tracking-[0.12em] text-[#E0B880]">{day.day}</p>
                <h3 className="mt-4 max-w-[680px] text-[27px] font-semibold leading-[1.18] text-[#403028] md:text-[31px]">{day.title}</h3>
                <p className="mt-6 max-w-[760px] text-[16px] font-semibold leading-[1.68] text-[#403028]">{day.description}</p>
                <div className="mt-7 border-t border-[#403028]/13 pt-6">
                  <h4 className="text-[22px] font-semibold leading-[1.18] text-[#403028]">Accommodations:</h4>
                  <p className="mt-3 max-w-[650px] text-[16px] font-semibold leading-[1.65] text-[#403028]">{day.accommodation}</p>
                </div>
              </div>
            </article>
          );
...
function ItineraryImage({ images, sizes, label, className = "" }: { images: { src: string; alt: string }[]; sizes: string; label?: string; className?: string }) {
  ...
  return (
    <div className={`relative min-h-[240px] overflow-hidden bg-[#403028] lg:min-h-full ${className}`}>
      {image ? (
        <Image src={image.src} alt={image.alt} fill sizes={sizes} loading="eager" unoptimized className="object-cover" />
      ) : null}
      {label ? <span className="absolute left-5 top-5 rounded-[8px] bg-white/92 px-5 py-3 text-[15px] font-bold leading-none text-[#403028] shadow-[0_12px_24px_rgba(64,48,40,0.12)]">{label}</span> : null}
    </div>
  );
}
  ...
  return (
    <section id="inclusions" className={`${pageContainer} scroll-mt-[82px] py-[48px] md:py-[72px]`}>
      <div className={excludedItems.length > 0 ? "mx-auto grid max-w-[930px] gap-10 text-left md:grid-cols-[minmax(0,1fr)_1px_minmax(0,1fr)] md:items-stretch md:gap-12" : "text-left"}>
        ...
        {excludedItems.length > 0 ? <div className="hidden w-px bg-[#403028]/15 md:block" aria-hidden="true" /> : null}
        {excludedItems.length > 0 ? (
          <div className="border-t border-[#403028]/13 pt-10 md:border-t-0 md:pt-0">
      ...
      <div id="pricing" className="scroll-mt-[82px] pt-[48px] md:pt-[82px] lg:col-span-2">
        ...
        <div className="mx-auto mt-3 flex w-full max-w-[1110px] flex-wrap justify-center gap-2">
          {priceTiers.map((tier) => (
            <div key={tier.id} className="min-h-[92px] w-full max-w-[286px] rounded-[8px] border border-[#403028]/10 bg-white px-4 py-5 text-center shadow-[0_16px_35px_rgba(64,48,40,0.06)] sm:w-[calc(50%-4px)] md:w-[calc(33.333%-6px)] lg:w-[178px]">
              <p className="text-[11px] font-semibold leading-none text-[#403028]/78">{tier.people}</p>
              <p className="mt-3 text-[20px] font-bold leading-none text-[#E0B880]">{tier.price}</p>
              <p className="mt-2 text-[11px] font-semibold leading-none text-[#403028]/68">per person</p>
            </div>
          ))}
        </div>
...
function InlinePlannerForm({ planner, className = "" }: { planner: HomeDictionary["planner"]; className?: string }) {
  ...
  return (
    <form className={`grid w-full max-w-[392px] gap-4 ${className}`} aria-label={planner.title} onSubmit={submitForm} noValidate>
      ...
      <button type="submit" disabled={submitting} className="flex h-[42px] items-center rounded-[6px] bg-[#E0B880] px-4 text-left text-[15px] font-semibold leading-[1.6] text-[#403028] transition hover:bg-[#C99A5D] focus:outline-none focus-visible:ring-2 focus-visible:ring-white/80 disabled:cursor-not-allowed disabled:opacity-70">
...
function InlinePlannerField({ field, value, invalid, onChange }: { field: PlannerField; value: string; invalid: boolean; onChange: (name: string, value: string) => void }) {
  const fieldClassName = "h-[47px] w-full rounded-[4px] border border-[#403028]/15 bg-[#FFF8F0]/98 px-4 text-[13px] font-bold leading-[1.4] text-[#403028] shadow-none outline-none backdrop-blur-[12px] placeholder:text-[#403028]/60 focus:border-[#E0B880] focus:ring-2 focus:ring-[#E0B880]/45";
    ...
    <section className="bg-[#403028] py-[48px] text-white md:py-[76px]">
      <div className={`${pageContainer} grid gap-10 md:grid-cols-[1fr_520px] md:items-center`}>
        <div>
          ...
          <h2 className="mt-4 text-[42px] font-semibold leading-[1.08]">Stop overthinking.<br /><span className="text-[#E0B880]">Start planning.</span></h2>
        ...
        {trips.slice(0, 3).map((trip, index) => (
          <Link key={`${trip.slug}-similar-${index}`} href={`/trip/${trip.slug}`} className="group overflow-hidden rounded-[8px] border border-[#403028]/10 bg-white shadow-[0_14px_34px_rgba(64,48,40,0.08)] transition hover:-translate-y-0.5 hover:shadow-[0_20px_44px_rgba(64,48,40,0.12)]">
            ...
            <div className="p-5">
              ...
              <p className="mt-2 text-[13px] font-semibold leading-[1.55] text-[#403028]/65">{trip.duration} - {trip.route}</p>
              <span className="mt-4 inline-flex items-center gap-2 text-[13px] font-bold text-[#E0B880]">See Itinerary <ArrowRight className="size-4" aria-hidden="true" /></span>
            </div>
          </Link>
        ))}
      </div>
...
function FaqSection({ dictionary, faqs }: { dictionary: HomeDictionary; faqs: { question: string; answer: string }[] }) {
  ...
  return (
    <section className="mx-auto max-w-[900px] px-6 py-[48px] md:py-[76px]">
      ...
      <div className="mt-8 border-y border-[#403028]/12">
        {faqs.map((faq, index) => (
          <button key={`${faq.question}-${index}`} type="button" onClick={() => setOpen(open === index ? -1 : index)} className="w-full border-b border-[#403028]/12 bg-transparent py-5 text-left last:border-b-0">
            <span className="flex items-center justify-between gap-4">
              <span className="text-[16px] font-medium leading-[1.5]">{faq.question}</span>
              <span className="grid size-8 shrink-0 place-items-center rounded-full border border-[#403028]/12 text-[#E0B880]"><ChevronDown className={`size-4 transition ${open === index ? "rotate-180" : ""}`} aria-hidden="true" /></span>
            </span>
            {open === index ? <span className="mt-4 block text-[15px] font-semibold leading-[1.6] text-[#403028]/62">{faq.answer}</span> : null}
          </button>
        ))}
      </div>
