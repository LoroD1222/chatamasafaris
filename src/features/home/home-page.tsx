        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3 md:items-stretch lg:gap-6">
          {dictionary.experienceCategories.items.map((item, index) => (
            <Link
              ...
              className={cn(
                "group flex h-full min-h-[520px] flex-col rounded-[10px] border border-[#403028]/20 bg-white p-2 shadow-[0_14px_40px_rgba(64,48,40,0.06)] outline-none transition hover:-translate-y-0.5 hover:shadow-[0_18px_50px_rgba(64,48,40,0.1)] focus-visible:ring-2 focus-visible:ring-astra-gold focus-visible:ring-offset-2",
          ...
          <div className="grid gap-[18px] md:grid-cols-2 lg:grid-cols-3">
            {recentTrips && recentTrips.length > 0
              ? recentTrips.map((trip, index) => (
                  <a key={`${trip.slug}-${index}`} href={`/trip/${trip.slug}`} className="group relative block h-[401px] overflow-hidden rounded-lg bg-astra-cocoa shadow-[0_18px_55px_rgba(64,48,40,0.15)] outline-none transition focus-visible:ring-2 focus-visible:ring-astra-gold focus-visible:ring-offset-2" aria-label={`See itinerary for ${trip.title}`}>
                    ...
                    <div className="absolute inset-x-0 bottom-0 min-h-[128px] rounded-b-lg bg-[#403028]/40 px-5 pb-[25px] pt-5 text-white backdrop-blur-sm">
...
function ItineraryCard({ locale, item }: { locale: Locale; item: Itinerary }) {
  return (
    <Link
      ...
      className="group relative block h-[401px] overflow-hidden rounded-lg bg-astra-cocoa shadow-[0_18px_55px_rgba(64,48,40,0.15)] outline-none transition focus-visible:ring-2 focus-visible:ring-astra-gold focus-visible:ring-offset-2"
