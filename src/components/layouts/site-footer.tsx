...
export function SiteFooter({ locale, dictionary }: { locale: Locale; dictionary: HomeDictionary }) {
  return (
    <footer className="bg-astra-cream text-astra-cocoa">
      <div className="container flex max-w-[1280px] flex-col items-center gap-[29px] py-16 md:py-[72px]">
        ...
        <nav className="flex flex-wrap justify-center gap-8 text-base font-medium leading-[1.5] tracking-[-0.18px] text-astra-brown/75 md:gap-16" aria-label="Footer">
          {dictionary.footer.links.map((item, index) => (
            <Link key={`${item.label}-${index}`} href={localizedHref(locale, item.href)} className="transition hover:text-astra-gold">
              {item.label}
            </Link>
          ))}
        </nav>
